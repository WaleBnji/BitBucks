        ...
        <div className=' w-[45%] flex flex-col  '>
          <div className='flex w-full align-center space-x-2 '>
            <img
              ...
              className='w-[60px]'
            />
            <h1 className='capitalize text-xl font-semibold'>{coinData?.id}</h1>
          ...
          <h2 className='text-[1.7rem] font-semibold'>
            {new Intl.NumberFormat('en-Ng', {
              style: 'currency',
              currency: currency,
            }).format(Number(coinData?.market_data?.current_price[currency]))}
          </h2>
            ...
            <div>
              ...
              <p className='text-[1.3rem] font-semibold'>
                {new Intl.NumberFormat('en-Ng', {
                  style: 'currency',
                  currency: currency,
                }).format(Number(coinData?.market_data?.market_cap[currency]))}
              </p>
            </div>
            <div>
          ...
          <div>
            ...
            <p className='text-[1.3rem] font-semibold'>
              {new Intl.NumberFormat('en-Ng', {
                style: 'currency',
                currency: currency,
              }).format(Number(coinData?.market_data?.total_volume[currency]))}
            </p>
          </div>
          <div className='flex align-center justify-between my-4'>
            <div>
            ...
            <div>
              ...
              <p className='text-[1.3rem] font-semibold'>
                {new Intl.NumberFormat('en-Ng', {
                  style: 'currency',
                  currency: currency,
                }).format(Number(coinData?.market_data?.high_24h[currency]))}
              </p>
            </div>
            <div>
              <span className='text-gray-100 text-sm'>Circulating volume</span>
              <p className='text-[1.3rem] font-semibold'>
                {new Intl.NumberFormat('en-Ng', {
                  style: 'currency',
                  currency: currency,
                }).format(Number(coinData?.market_data?.circulating_supply))}
              </p>
            </div>
          </div>
          <div className='flex justify-between'>
            <div className='flex flex-col space-y-2 justify-center'>
              ...
              <div className='bg-gray-200 bg-opacity-50 text-gray-100 text-sm rounded px-2 py-1'>
                <a target='blank' href={coinData?.links?.official_forum_url[0]}>
                  {coinData?.links?.official_forum_url[0]}
                </a>
              </div>
              <div className='bg-gray-200 bg-opacity-50 text-gray-100 text-sm rounded px-2 py-1'>
                <a target='blank' href={coinData?.links?.blockchain_site[0]}>
